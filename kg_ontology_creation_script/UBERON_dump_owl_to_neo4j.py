# -*- coding:utf-8 -*-
#This script help to load uberon information into neo4j
import sys
import py2neo
import numpy as np
import pandas as pd
...
if __name__ == "__main__":
	...
	try:
		graph.schema.create_uniqueness_constraint("Anatomy", "url")
	except:
		pass
	...
	key_subset = set(("subClassOf","url","name","definition","hasAlternativeId","cross reference","id"))
	...
	with open(target_file, "r") as f:
		for idx, line in tqdm(enumerate(f),total=num_file):
			line_s = line.rstrip("\n").split("\t")
			target_node = Node("Anatomy")
			target_node["source"] = "Uber-anatomy ontology"
			nodel_url = ""
			node_subclassOf = []
			for info in line_s:
			...
			if "url" in target_node:
				if not node_matcher.match("Anatomy",url=target_node["url"]):
					tx = graph.begin()
	...
	for s_node_url, o_node_url_list in tqdm(target_node_dict.items(),total=target_node_dict_len):
		s_node = node_matcher.match("Anatomy", url=s_node_url).first()
		for o_node_url in o_node_url_list:
			o_node = node_matcher.match("Anatomy",url=o_node_url).first()
			if s_node and o_node:
				p_relationship_name = "subClassOf"
				p_relationship = Relationship(s_node, p_relationship_name, o_node, source="UBERON")
				subgraph = s_node | p_relationship | o_node
				tx = graph.begin()
				tx.merge(subgraph, "Anatomy", "url")
				graph.commit(tx)
			else:
