# -*- coding:utf-8 -*-
#This script help to load ontology information into neo4j
...
if __name__ == "__main__":
	...
	try:
		graph.schema.create_uniqueness_constraint("Compound", "url")
	except:
		pass
	...
	#compound attribute
	key_subset = set(("subClassOf","url","name","definition","hasAlternativeId","cross reference","id","inchi",\
		"inchikey","formula"))

	num_file = sum([1 for i in open(target_file,"r")])
	target_node_dict = {}
	#reading information from ontology
	with open(target_file, "r") as f:
		for idx, line in tqdm(enumerate(f),total=num_file):
			line_s = line.rstrip("\n").split("\t")
			target_node = Node("Compound")
			target_node["source"] = "Chemical Entities of Biological Interest"
			nodel_url = ""
			node_subclassOf = []
			for info in line_s:
			...
			if "url" in target_node:
				#loading info
				if not node_matcher.match("Compound",url=target_node["url"]):
					tx = graph.begin()
	...
	for s_node_url, o_node_url_list in tqdm(target_node_dict.items(),total=target_node_dict_len):
		s_node = node_matcher.match("Compound", url=s_node_url).first()
		for o_node_url in o_node_url_list:
			o_node = node_matcher.match("Compound",url=o_node_url).first()
			if s_node and o_node:
				p_relationship_name = "subClassOf"
				p_relationship = Relationship(s_node, p_relationship_name, o_node, source="CHEBI")
				subgraph = s_node | p_relationship | o_node
				tx = graph.begin()
				tx.merge(subgraph, "Compound", "url")
				graph.commit(tx)
			else:
