# -*- coding:utf-8 -*-
#This script help to load gene information from OGG ontology into neo4j
import sys
import py2neo
import numpy as np
import pandas as pd
...
if __name__ == "__main__":
	...
	try:
		graph.schema.create_uniqueness_constraint("Gene", "url")
	except:
		pass
	...
	key_subset = set(("subClassOf","url","name","definition","hasAlternativeId","cross reference","entrez id",\
		"gene symbol","alternative term","chromosome","organism NCBITaxon ID","description","type of gene"))
	...
	with open(target_file, "r") as f:
		for idx, line in tqdm(enumerate(f),total=num_file):
			line_s = line.rstrip("\n").split("\t")
			target_node = Node("Gene")
			target_node["source"] = "OGG: Ontology of Genes and Genomes"
			nodel_url = ""
			node_subclassOf = []
			for info in line_s:
			...
			if "url" in target_node:
				if not node_matcher.match("Gene",url=target_node["url"]):
					tx = graph.begin()
	...
	for s_node_url, o_node_url_list in tqdm(target_node_dict.items(),total=target_node_dict_len):
		s_node = node_matcher.match("Gene", url=s_node_url).first()
		for o_node_url in o_node_url_list:
			if o_node_url != s_node_url:
				o_node = node_matcher.match("Gene",url=o_node_url).first()
				if s_node and o_node:
					p_relationship_name = "subClassOf"
					p_relationship = Relationship(s_node, p_relationship_name, o_node, source="OGG: Ontology of Genes and Genomes")
					subgraph = s_node | p_relationship | o_node
					tx = graph.begin()
					tx.merge(subgraph, "Gene", "url")
					graph.commit(tx)
				else:
