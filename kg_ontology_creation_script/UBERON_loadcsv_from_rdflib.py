...
def uberon_pattern_match():
	pattern = regex.compile("http://purl.obolibrary.org/obo/((UBERON)|(CL))_[0-9]*")
	return pattern
...
if __name__ == "__main__":
	...
	uberon_class_dict = uberon_dict_setttings()
		...
		for s,p,o in tqdm(res_graph.triples((None,None,OWL.Class))):
			if pattern.match(s):
				for s1,p1,o1 in res_graph.triples((s,None,None)):
					...
					if p1_predication in uberon_class_dict:
						O.write("|".join([uberon_class_dict[p1_predication],o1])+"\t")
